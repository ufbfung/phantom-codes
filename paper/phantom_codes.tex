...
%   - vancouver:    superscripted   — biomedical (JAMIA, BMJ) ← active
%   - numeric-comp: [1, 2]          — IEEE, Nature, NeurIPS
%   - authoryear:   (Devlin, 2019)  — skim-friendly drafting style
%
% Active style: vancouver (JAMIA Research and Applications uses
% AMA-family superscripted numeric refs).
...
% ─── Review-mode niceties ─────────────────────────────────────────────
% Line numbers on the submitted PDF let reviewers reference specific
% lines; comment-out for read-throughs, uncomment if a venue requires.
% \usepackage{lineno}
% \linenumbers
% Double-spacing per JAMIA submission convention.
\usepackage{setspace}
\doublespacing
...
on medical concept normalization, separating fabrication of non-
existent codes from abstention as failure modes, and to identify
the (model, prompting-mode) configurations that combine high
accuracy with low per-prediction cost.
...
Zero-shot LLM prompting is inferior to constrained and retrieval-
augmented grounding for medical concept normalization. Among grounded configurations,
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2000}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2000}}@{}}
\caption{Failure-mode breakdown by model and degradation mode. Each cell reports hallucination \% / no\_prediction \%.}\tabularnewline
\toprule()
...
\caption{Top-1 vs top-5 exact-match lift across selected configurations.}\tabularnewline
\toprule()
...
\caption{Per-call API cost across selected (model, prompting-mode) configurations from the headline matrix. n = number of completed calls (out of 500 attempted). †, ‡ explained in the footnotes below.}\tabularnewline
\toprule()
...
\caption{Cost per correct prediction. \emph{constr.} = constrained. *Gemini 2.5 Pro constrained reflects abstention pattern, not error rate; see §4.3 (Failure-mode breakdown).}\tabularnewline
\toprule()
\begin{minipage}[b]{\linewidth}\raggedright
Model
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
