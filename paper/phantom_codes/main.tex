...
% IEEEtran's \IEEEPARstart drop-cap macro hard-codes a Type 1 font
% lookup ("ptmrq8r") that XeLaTeX + fontspec cannot resolve. Override
% it to use our main font at a larger scale, preserving the drop-cap
% effect without the font-resolution failure.
\renewcommand{\IEEEPARstart}[2]{{\fontsize{40pt}{42pt}\selectfont #1}#2}
...
\author{Brian~K.~Fung,~PharmD,~MPH%
...
% ─── Author biography (IEEE convention) ────────────────────────────────
% Single author, no photograph supplied. IEEE convention is to include
% a brief bio at the end of the article; for venues that allow no
% photograph, IEEEbiographynophoto is the right environment.
\begin{IEEEbiographynophoto}{Brian K. Fung}
(PharmD, MPH) is an independent researcher based in Brandon, Florida,
USA. He holds a Doctor of Pharmacy (PharmD) and a Master of Public
Health (MPH), and his research focuses on the safe and cost-effective
deployment of large language models in healthcare, with current
emphasis on medical concept normalization, semantic interoperability,
and reproducible evaluation infrastructure for clinical NLP. He
