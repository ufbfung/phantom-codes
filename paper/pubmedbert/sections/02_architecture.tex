...
\autocite{Devlin2019}; we make three deliberate choices on top of it.

\subsection{Multi-label rather than single-label}\label{multi-label-rather-than-single-label}
...
(\texttt{BCEWithLogitsLoss} for numerical stability). While our v1 ground
...
\subsection{Top-N vocabulary cap}\label{top-n-vocabulary-cap}
...
The long tail of rare codes ($\leq$10 instances in the train split)
...
extreme classification \autocite{Chalkidis2020}; we err toward the
well-supported subset for v1.

\subsection{Single linear head, no MLP}\label{single-linear-head-no-mlp}
...
\subsection{Choice of base encoder: PubMedBERT}\label{choice-of-base-encoder-pubmedbert}

We use \textbf{PubMedBERT-base-uncased-abstract-fulltext} \autocite{Gu2021} as
the pre-trained encoder for three reasons:

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\item
  \textbf{Domain-adaptive pre-training.} PubMedBERT is trained from
  ...
  fragmented subwords \autocite{Devlin2019,Gururangan2020}.
\item
  \textbf{Strong empirical performance} on the BLURB biomedical NLP
  benchmark \autocite{Gu2021}.
\item
  \textbf{Computational footprint} (\textasciitilde110M parameters) compatible with
...
\tightlist
\item
  \textbf{ClinicalBERT} \autocite{Alsentzer2019} --- ruled out because its
  pre-training corpus includes MIMIC-III, which contaminates
  MIMIC-IV evaluation.
\item
  \textbf{BioBERT} \autocite{Lee2020} and \textbf{BioLinkBERT} \autocite{Yasunaga2022} ---
  comparable scale to PubMedBERT, deferred to a future ablation
  arm.
\item
  \textbf{Larger biomedical models} (BiomedLM, GatorTron, BioMistral,
  Med-PaLM) via parameter-efficient fine-tuning \autocite{Hu2022,Dettmers2023} --- deferred; would require either cloud GPU or
  significantly more local memory.
\end{itemize}

Generative LLMs (Claude, GPT, Gemini) are evaluated separately in
the companion paper \autocite{FungPhantomCodes2026} as zero-shot /
